...
#else
                Printer::currentPositionSteps[Z_AXIS] = (sum + ENDSTOP_Z_BACK_ON_HOME) * Printer::axisStepsPerMM[Z_AXIS];
...
*/
#endif // DRIVE_SYSTEM
		case 156:	//G156 goto Yaxial max
			Printer::homeAxis(false,true,false);
			Printer::moveToReal(IGNORE_COORDINATE,Printer::yLength,IGNORE_COORDINATE,IGNORE_COORDINATE,Printer::homingFeedrate[Y_AXIS]);
			break;
        }
        previousMillisCmd = HAL::timeInMilliseconds();
    }
...
#if FEATURE_Z_PROBE
void float2str(float number, char *outstring)
{
	char strbuf[8] = "0.0";
	uint8_t n=0;
	
	#if AUTO_LEVEL_DEBUG
	Com::printFLN(PSTR("=====flaot2str()====="));
	Com::printFLN(PSTR("number = "),number);
	Com::printFLN(PSTR("[1]strbuf ="));
	Com::print(strbuf);
	#endif
	
	if (isnan(number)) {		
		strcpy(outstring, strbuf);

		#if AUTO_LEVEL_DEBUG
		Com::printFLN(PSTR("[2]outstring ="));
		Com::print(outstring);
		#endif
	...
	if(int_part > 10)
	{
		strbuf[n++] = '0'+(int_part/10)%10;
	}
	strbuf[n++] = '0'+(int_part/1)%10;
	// Print the decimal point, but only if there are digits beyond
	strbuf[n++] = '.';  	
	// Extract digits from the remainder one at a time
	remainder *= 10.0;
	int toPrint = int(remainder);
	strbuf[n++] = '0'+(toPrint/1)%10;
	remainder -= toPrint;
	...
	#if AUTO_LEVEL_DEBUG
	Com::printFLN(PSTR("[4]strbuf ="));
	Com::print(strbuf);
	Com::printFLN(PSTR("[4]outstring ="));
	Com::print(outstring);
	#endif
	return ;	
}

void Commands::BedLevelingMease()
{	
	uint8_t i;
	char str[20]="";
	char strbuf[8]="";	
	float base=0, sum = 0, last, oldFeedrate = Printer::feedrate;
	float fError[3], fErrorMin=0, fErrorMax=0;
	
	uid.executeAction(UI_ACTION_TOP_MENU);
	UI_CLEAR_STATUS

	UI_STATUS_UPD_RAM(UI_TEXT_BED_LEVEL_MEAS);
	GCode::executeFString(Com::tZProbeStartScript);
	bool oldAutolevel = Printer::isAutolevelActive();
	Printer::setAutolevelActive(false);	
	
	//0
	Printer::moveTo(Z_PROBE_X0,Z_PROBE_Y0,IGNORE_COORDINATE,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
	base = Printer::runZProbe(false,false,Z_PROBE_REPETITIONS,false);
	Com::printFLN(PSTR("base Z Probe height= "),base);
	if(Printer::currentPosition[Z_AXIS] <= (ENDSTOP_Z_BACK_ON_HOME+Z_PROBE_SWITCHING_DISTANCE))
	{
		Printer::feedrate = oldFeedrate;
		Printer::setAutolevelActive(oldAutolevel);
		Printer::updateCurrentPosition(true);
		printCurrentPosition();	
		Printer::kill(true);
		UI_STATUS_UPD_RAM(UI_TEXT_ZPROBE_TOOHIGH);
		return;
	}
	
	///1
	Printer::moveTo(EEPROM::zProbeX1(),EEPROM::zProbeY1(),IGNORE_COORDINATE,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
	fError[0] = Printer::runZProbe(false,false,Z_PROBE_REPETITIONS,false)-base;
	Com::printFLN(PSTR("1st Z Probe height= "),fError[0]);
	//if(sum<0) return;
	float2str(fError[0], strbuf);
	strcpy(str, strbuf);
	//Com::printF(PSTR("[1st]str ="));
	//Com::printFLN(str);
	UI_STATUS_UPD_RAM(str);
	sum+= fError[0];
	
	///2
	Printer::moveTo(EEPROM::zProbeX2(),EEPROM::zProbeY2(),IGNORE_COORDINATE,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
	fError[1] = Printer::runZProbe(false,false)-base;
	Com::printFLN(PSTR("2nd Z Probe height= "),fError[1]);
	//if(last < 0) return;
	float2str(fError[1], strbuf);
	strcat(str, strbuf);
	//Com::printF(PSTR("[2nd]str ="));
	//Com::printFLN(str);
	UI_STATUS_UPD_RAM(str);
	sum+= fError[1];
	...
	///3
	Printer::moveTo(EEPROM::zProbeX3(),EEPROM::zProbeY3(),IGNORE_COORDINATE,IGNORE_COORDINATE,EEPROM::zProbeXYSpeed());
	fError[2] = Printer::runZProbe(false,true)-base;	
	Com::printFLN(PSTR("3rd Z Probe height= "),fError[2]);
	//if(last < 0) return;
	float2str(fError[2], strbuf);
	strcat(str, strbuf);
	//Com::printF(PSTR("[3rd]str ="));
	//Com::printFLN(str);
	UI_STATUS_UPD_RAM(str);	
	sum+= fError[2];
	...
	fErrorMin = fErrorMax = fError[0];
	for(i=1; i<3; i++)
	{
		if(fError[i] < fErrorMin)
			fErrorMin = fError[i];
		if(fError[i] > fErrorMax)
			fErrorMax = fError[i];
	}	
	Printer::feedrate = oldFeedrate;
	Printer::setAutolevelActive(oldAutolevel);
	...
	Printer::kill(true);
	UI_STATUS_UPD_RAM(str);	
	if(((fErrorMax - fErrorMin) <= 0.2) && (abs(fErrorMax) <=0.2) && (abs(fErrorMin) <=0.2))
	{
		uid.executeAction(UI_ACTION_TOPMENU);
		HAL::delayMilliseconds(3000);
		UI_CLEAR_STATUS		
		UI_STATUS_UPD_RAM(UI_TEXT_BED_LEVEL_PASS);
	}	
}
#endif
