...
float Printer::runZProbe(bool first,bool last,uint8_t repeat,bool runStartScript)
{
	...
	#if AUTO_LEVEL_DEBUG
	Com::printFLN(PSTR("runZProbe++++++++2!"));
	#endif
	
    for(uint8_t r=0; r<repeat; r++)
    {
    	#if AUTO_LEVEL_DEBUG
		Com::printFLN(PSTR("loop = "),r);
		#endif
		
        //probeDepth = 2 * (Printer::zMaxSteps - Printer::zMinSteps);
		probeDepth = 10 * axisStepsPerMM[Z_AXIS];
		
		#if AUTO_LEVEL_DEBUG
		Com::printFLN(PSTR("probeDepth = "),probeDepth);
		#endif
		
		#if NONLINEAR_SYSTEM
        stepsRemainingAtZHit = -1;
		#else
		stepsRemainingAtZHit = 0;
		#endif
		...
		#if AUTO_LEVEL_DEBUG
		Com::printFLN(PSTR("runZProbe++++++++3!"));
		#endif
		
        waitForZProbeStart();

		#if AUTO_LEVEL_DEBUG
		Com::printFLN(PSTR("runZProbe++++++++4!"));
		#endif
		
        setZProbingActive(true);
		
		#if AUTO_LEVEL_DEBUG
		Com::printFLN(PSTR("runZProbe++++++++5!"));
		#endif
	...
	#if AUTO_LEVEL_DEBUG
	Com::printFLN(PSTR("runZProbe++++++++8! lastCorrection="), lastCorrection);
	Com::printFLN(PSTR("runZProbe++++++++9! currentPositionSteps[Z_AXIS]="), currentPositionSteps[Z_AXIS]);	
	#endif
