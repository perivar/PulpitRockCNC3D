...
void PrintLine::moveRelativeDistanceInStepsReal(long x,long y,long z,long e,float feedrate,bool waitEnd)
{
	...
	#else
	Printer::moveToReal((x==0)?IGNORE_COORDINATE:Printer::lastCmdPos[X_AXIS],\
						(y==0)?IGNORE_COORDINATE:Printer::lastCmdPos[Y_AXIS],\
						(z==0)?IGNORE_COORDINATE:Printer::lastCmdPos[Z_AXIS],\
                        (Printer::currentPositionSteps[E_AXIS] + e) * Printer::invAxisStepsPerMM[E_AXIS],\
                        feedrate);
	#endif
    Printer::updateCurrentPosition();
    if(waitEnd)
